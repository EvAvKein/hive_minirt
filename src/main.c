...
void	gradually_render(t_data *data, size_t i0, size_t precision)
{
	...
	while (precision > 0)
	{
		...
		precision /= 2;
		if (data->threads_waiting != 8)
			++data->threads_waiting;
		while (!(data->threads_waiting == 8 || data->threads_waiting == 0
				|| data->stop))
			usleep(500);
		if (data->stop)
			break ;
		if (data->threads_waiting == 0)
			continue ;
		data->threads_waiting = 0;
	}
}
